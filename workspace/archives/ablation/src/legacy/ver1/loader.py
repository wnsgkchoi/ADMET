...
def get_maccs_features(mol):
    """Calculate MACCS Keys (167 bits)"""
    fp = MACCSkeys.GenMACCSKeys(mol)
    return np.array(fp, dtype=np.float32)

def get_ecfp_features(mol, radius=2, nBits=1024):
    """Calculate ECFP4 (1024 bits)"""
...
def mol_to_graph_data_obj_simple(mol, smile=None, feature_flags=None):
    """
    Converts rdkit mol object to graph Data object required by the pytorch
    geometric package. Uses extended atom and bond features for better ADMET prediction.
    :param mol: rdkit mol object
    :param feature_flags: list of strings, e.g. ['phys', 'maccs', 'ecfp']
    :return: graph data object with the attributes: x, edge_index, edge_attr
    """
    # atoms - Extended features for ADMET prediction
    # Features: atomic_num, chirality, degree, formal_charge, num_h, hybridization, is_aromatic
    num_atom_features = 7
    atom_features_list = []
    ...
    # Calculate global features if requested
    global_feats = []
    if feature_flags:
        if 'phys' in feature_flags:
            global_feats.append(get_admet_features(mol))
        if 'maccs' in feature_flags:
            global_feats.append(get_maccs_features(mol))
        if 'ecfp' in feature_flags:
            global_feats.append(get_ecfp_features(mol))
            
    if global_feats:
        global_feats = np.concatenate(global_feats)
        # Repeat for each node
        num_nodes = len(atom_features_list)
        global_feats_repeated = np.tile(global_feats, (num_nodes, 1))
        
        x_basic = np.array(atom_features_list, dtype=np.float32)
        x_combined = np.concatenate([x_basic, global_feats_repeated], axis=1)
        x = torch.tensor(x_combined, dtype=torch.float)
    else:
        x = torch.tensor(np.array(atom_features_list), dtype=torch.long)

    # bonds - Extended features for GNN
    # Features: bond_type, bond_direction, is_in_ring, is_conjugated
    num_bond_features = 4
...
def graph_data_obj_to_mol_simple(data_x, data_edge_index, data_edge_attr):
    ...
    for i in range(num_atoms):
        ...
        chirality_tag = allowable_features['possible_chirality_list'][chirality_tagIdx]
...
def graph_data_obj_to_nx_simple(data):
    ...
    for i in range(num_atoms):
        atomic_num_idx, chirality_tag_idx = atom_features[i]
        G.add_node(i, atom_num_idx=atomic_num_idx, chirality_tag_idx=chirality_tagIdx)
        pass
...
def nx_to_graph_data_obj_simple(G):
    ...
    are numbered from 0 to num_nodes - 1. Uses extended atom and bond features.
    NB: possible issues with recapitulating relative stereochemistry since the
    edges in the nx object are unordered.
    :param G: nx graph obj
    :return: pytorch geometric Data object
    """
    # atoms - Extended features (need to be stored in nx graph)
    num_atom_features = 7
    atom_features_list = []
    for _, node in G.nodes(data=True):
        atom_feature = [
            node.get('atom_num_idx', 0),
            node.get('chirality_tag_idx', 0),
            node.get('degree_idx', 0),
            node.get('formal_charge_idx', 5),  # default to 0 charge (index 5)
            node.get('num_h_idx', 0),
            node.get('hybridization_idx', 6),  # default to UNSPECIFIED
            node.get('is_aromatic', 0)
        ]
        atom_features_list.append(atom_feature)
    x = torch.tensor(np.array(atom_features_list), dtype=torch.long)

    # bonds - Extended features
    num_bond_features = 4
    if len(G.edges()) > 0:  # mol has bonds
        ...
        for i, j, edge in G.edges(data=True):
            edge_feature = [
                edge.get('bond_type_idx', 0),
                edge.get('bond_dir_idx', 0),
                edge.get('is_in_ring', 0),
                edge.get('is_conjugated', 0)
            ]
            edges_list.append((i, j))
            edge_features_list.append(edge_feature)
            edges_list.append((j, i))
...
class MoleculeCSVDataset(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None,
                 dataset_name='custom', smiles_col='smiles', target_cols=None, feature_flags=None):
        ...
        super(MoleculeCSVDataset, self).__init__(root, transform, pre_transform, pre_filter)
        
        self.data, self.slices = torch.load(self.processed_paths[0], weights_only=False)

    @property
    def raw_file_names(self):
        # This is not strictly needed anymore as we construct the path directly in `process`.
        return []
    ...
    def processed_file_names(self):
        ...
        if self.feature_flags:
            ...
            flags_str = '_'.join(sorted(self.feature_flags))
            suffix = f'_{flags_str}'
        return [f'{self.dataset_name}_processed{suffix}.pt']

    def download(self):
        # This dataset is not downloaded, but loaded from a local CSV file.
        pass
    ...
    def process(self):
        ...
        if not os.path.exists(raw_path):
            ...
            filename_map = {
                'dili2': 'dili_binary.csv',
                'dili3': 'dili_3class.csv',
                'hk2': 'hk2_ic50.csv',
                'hepg2': 'hepg2.csv'
            }
            filename = filename_map.get(self.dataset_name)
            if filename:
                raw_path = os.path.join(self.root, filename)
        ...
        # Drop rows with missing values in smiles or target columns
        df.dropna(subset=[self.smiles_col] + self.target_cols, inplace=True)
        ...
        for i, row in df.iterrows():
            ...
            if mol is None:
                print(f"Warning: RDKit could not parse SMILES: {smiles}. Skipping.")
                continue

            data = mol_to_graph_data_obj_simple(mol, smiles, feature_flags=self.feature_flags)
            
            # Add target(s)
            if self.target_cols:
                y = torch.tensor([row[self.target_cols]], dtype=torch.float)
                data.y = y
            ...
            if scaffold not in all_scaffolds:
                all_scaffolds[scaffold] = scf_idx_counter
                scf_idx_counter += 1
            
            data.scf_idx = torch.tensor([all_scaffolds[scaffold]])
...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.dataset == 'tox21':
            ...
            # ... (rest of the original code for tox21)
        # ... (rest of the original MoleculeDataset class)
...
def _load_dili3_dataset(input_path):
    """
...
def load_admet_dataset(category, dataset_name, config_path='configs/dataset_config.json'):
    """
    Load ADMET dataset from the new structure: workspace/data/Category/Dataset_Name/
    
    Args:
        category: One of 'Absorption', 'Distribution', 'Metabolism', 'Excretion', 'Toxicity'
        dataset_name: Dataset name (e.g., 'Caco2_Wang', 'AMES', etc.)
        config_path: Path to dataset_config.json
    
    Returns:
        dict with keys:
            - 'train': DataFrame with columns [Drug_ID, Drug (SMILES), Y]
            - 'valid': DataFrame
            - 'test': DataFrame
            - 'full': DataFrame (all data combined)
            - 'metadata': dict with task_type, num_classes, metric, use_kfold, etc.
    """
    import json
    ...
    # Validate category
    if dataset_info['category'] != category:
        raise ValueError(f"Category mismatch: {dataset_name} is in {dataset_info['category']}, not {category}")
    ...
    for split in ['train', 'valid', 'test']:
        csv_path = f"{base_path}/{split}.csv"
        if os.path.exists(csv_path):
            data[split] = pd.read_csv(csv_path)
        else:
            raise FileNotFoundError(f"Missing {split}.csv at {csv_path}")
    
    # Load full data
    full_path = f"{base_path}/full_data.csv"
    if os.path.exists(full_path):
        data['full'] = pd.read_csv(full_path)
    else:
        # Combine train+valid+test if full_data doesn't exist
        data['full'] = pd.concat([data['train'], data['valid'], data['test']], ignore_index=True)
...
def parse_metadata_file(metadata_path):
    """
    Parse metadata.txt file to extract task type and other information.
    ...
    # Extract task type
    task_match = re.search(r'Task Type:\s*(\w+)', content)
    if task_match:
        info['task_type'] = task_match.group(1)
    ...
    name_match = re.search(r'Dataset Name:\s*(.+)', content)
    if name_match:
        info['dataset_name'] = name_match.group(1).strip()
