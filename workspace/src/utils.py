...
### utils for eval
def cal_roc(y_true, y_scores, num_classes=2):
    """
    Calculate ROC AUC score for binary or multi-class classification.
    
    Args:
        y_true: For binary: (N, num_tasks) array with 0/1 labels
                For multi-class: (N,) array with class indices 0, 1, 2, ...
        y_scores: For binary: (N, num_tasks) array with probabilities
                  For multi-class: (N, num_classes) array with class probabilities
        num_classes: Number of classes (2 for binary, >2 for multi-class)
    
    Returns:
        ROC AUC score (percentage 0-100)
    """
    if num_classes == 2:
        ...
        if len(roc_list) == 0:
            return 0
        return sum(roc_list) / len(roc_list) * 100
    else:
        # Multi-class classification
        # y_true: (N,) with class indices 0, 1, 2, ...
        # y_scores: (N, num_classes) with class probabilities
        return roc_auc_score(y_true, y_scores, multi_class='ovr') * 100
