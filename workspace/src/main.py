...
#dataset
    ...
    parser.add_argument('--benchmark_split_file', type=str, default='', 
                        help='Path to benchmark split pickle file (overrides default splitting)')
...
## output control
    ...
    parser.add_argument('--features', type=str, nargs='+', default=['basic'],
                        help='List of features to use: basic, phys, maccs, ecfp')
    
    args = parser.parse_args()

    # Calculate extra_feature_dim based on features
    extra_dim = 0
    if args.features:
        if 'phys' in args.features:
            extra_dim += 37
        if 'maccs' in args.features:
            extra_dim += 167
        if 'ecfp' in args.features:
            extra_dim += 1024
    args.extra_feature_dim = extra_dim
...
def save_results_to_csv(args, val_metric, test_metric, val_std=None, test_std=None, num_epochs=None, early_stopped=False, 
    ...
    # Add feature flags to result
    if hasattr(args, 'features') and args.features:
        result['features'] = '+'.join(sorted(args.features))
    else:
...
def main(args, trial=None):
    ...
    if args.benchmark_split_file:
        # Load benchmark data using pre-defined splits
        split_dir = os.path.dirname(args.benchmark_split_file)
        
        # Initialize dataset with full data
        # We use the directory of the split file as root, assuming data is there too
        full_dataset = MoleculeCSVDataset(
            root=split_dir,
            dataset_name=args.dataset_name, # e.g. 'AMES'
            smiles_col='Drug',
            target_cols=['Y'],
        ...
        # Load splits
        with open(args.benchmark_split_file, 'rb') as f:
            splits = pickle.load(f)
            
        # Determine seed key (seed_1 to seed_5)
        # Map args.seed (0-4) to seed_1-5
        seed_key = f"seed_{args.seed + 1}"
        
        if seed_key not in splits:
             raise ValueError(f"Seed {args.seed} not found in benchmark splits (available: {list(splits.keys())})")
                 
        qprint(f"Using benchmark split: {seed_key}", args)
        split_indices = splits[seed_key]
    ...
    elif args.category and args.dataset_name:
        ...
        dataset = MoleculeCSVDataset(root=args.dataset_dir, dataset_name=args.dataset, smiles_col='smiles', target_cols=target_cols, feature_flags=args.features)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if use_test_for_tracking:
                ...
                if is_better:
                    ...
                    if not args.no_save_model:
                        # Only save model if not in hyperparameter tuning mode (experiment_id is '0')
                        # Or if explicitly requested.
                        # For ablation study (experiment_id starts with 'ablation'), we might want to skip saving full models to save space
                        # unless it's the absolute best. But here we are inside the loop.
                        
                        # User requested: "hyperparameter tuning을 할 때에는 best_model을 저장하면 안 돼"
                        # We can use the existing --no_save_model flag for this.
                        # The command generator already adds --no_save_model if configured.
                        # But let's double check the logic here.
