...
class GNN_topexpert(torch.nn.Module): # expert 를 parallel 하게
    ...
    def __init__(self, args, criterion):
        ...
        self.experts_w = nn.Parameter(torch.empty(self.input_dim, output_dim))
    ...
    def forward(self, data):
        ...
        if self.extra_feature_dim > 0:
            x_cont = x[:, 7:]
            # Pool extra features (they are repeated per node, so mean gives the unique value)
            x_cont_graph = global_mean_pool(x_cont, batch)
            x_cont_graph = self.feat_bn(x_cont_graph)
            x_cont_emb = self.feat_proj(x_cont_graph)
            
            # Concatenate
            gnn_out = torch.cat([gnn_out, x_cont_emb], dim=1)
            gate_input = torch.cat([gate_input, x_cont_emb], dim=1)

        ## multi-head mlps
        gnn_out = torch.unsqueeze(gnn_out, -1) # N x input_dim x 1
